...
public sealed class MacroRecorder
{
    ...
    { _events.Clear(); _windows = windows.ToDictionary(window => window.WindowHandle); _clock.Restart(); }

    public bool TryRecord(RecorderWindow window, MacroEvent input, int clientX, int clientY, bool injected, bool multiWindow)
    {
        if (injected || !_windows.ContainsKey(window.WindowHandle)) return false;
    ...
    public IReadOnlyList<MacroEvent> Stop() { _clock.Stop(); return _events.OrderBy(item => item.OffsetMicroseconds).ToArray(); }
}
